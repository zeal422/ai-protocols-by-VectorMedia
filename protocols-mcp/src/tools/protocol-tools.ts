...
import { buildWorkflow, formatWorkflow, getWorkflowShortcuts, type TaskType } from '../search/workflow-builder.js';
...
import { RiskAssessmentEngine, RiskLevel, type CodeChange } from '../adaptation/risk-assessment.js';
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request: Record<string, unknown>) => {
    ...
    try {
      ...
      case 'route_task': {
        ...
        let inferredTaskType: TaskType = analyzeTaskIntent(description);
        if (overrideTaskType) {
          // Validate override is a valid task type
          const validTypes: TaskType[] = ['debug', 'build', 'refactor', 'audit', 'optimize', 'test', 'setup'];
          if (validTypes.includes(overrideTaskType as TaskType)) {
            inferredTaskType = overrideTaskType as TaskType;
          } else {
        ...
        response += formatWorkflow(workflow, inferredTaskType as TaskType);
          
        // Add shortcuts
        const shortcuts = getWorkflowShortcuts(inferredTaskType as TaskType);
      ...
      case 'build_adaptive_workflow': {
        ...
        const context: ProjectContext = inputContext ? {
          language: (inputContext.language as Language) || Language.Unknown,
          framework: (inputContext.framework as Framework) || Framework.None,
          projectType: (inputContext.projectType as ProjectType) || ProjectType.Unknown,
        ...
        const workflow = await workflowEngine.buildAdaptiveWorkflow(
          { description: taskDescription, type: taskType, priority: 'medium' },
          context,
          previousResults?.map(r => ({
            ...
            timestamp: new Date(),
            success: r.success,
            findings: r.findingsCount ? Array(r.findingsCount).fill({ findingId: 'test', severity: 'medium' as const, category: 'test', title: 'Test', description: 'Test', recommendations: [], location: { file: '', line: 0 }, metadata: {} }) : [],
            recommendations: [],
            artifacts: [],
            nextSteps: [],
      ...
      case 'get_execution_alerts': {
        ...
        const alerts = await riskAssessmentEngine.getExecutionAlerts(sessionId, level as RiskLevel | undefined);
